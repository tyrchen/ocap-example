...
copy-docs: $(RELEASE_DIR)
	@cd $(OUTPUT_DIR); tar zcf ocap_docs.tgz *; mv ocap_docs.tgz ../src/_build/releases/;

build-local:
	@echo "Building the dev release..."
	@cd src; mix deps.get; MIX_ENV=staging mix compile;
	@cd src/assets; npm install; npm run deploy;
	@cd src; MIX_ENV=staging mix phx.digest; MIX_ENV=staging mix release --env=local_staging
...
build-staging:
	...
	@cd src; mix deps.get; MIX_ENV=staging mix compile;
	@cd src/assets; npm install; npm run deploy;
	@cd src; MIX_ENV=staging mix phx.digest; MIX_ENV=staging mix release --env=staging
...
build-prod:
	...
	@cd src; mix deps.get; MIX_ENV=prod mix compile;
	@cd src/assets; npm install; npm run deploy;
	@cd src; MIX_ENV=prod mix phx.digest; MIX_ENV=prod mix release --env=prod
